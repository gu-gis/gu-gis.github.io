---
title: "Week 12: Working with OpenStreetMap Data and Data in Raster Format"
...
```{r}
#| label: taipei-mapview
# tpc_bb <- taipei_bb_results |> filter(place_id == 213710844)
tpc_bb <- taipei_bb_results |> filter(place_id == taipei_bb_results$place_id[1])
tpc_bb |> mapview()
```
```{r}
#| labe: ntpc-mapview
# ntpc_bb <- taipei_bb_results |> filter(place_id == 212454050)
ntpc_bb <- taipei_bb_results |> filter(place_id == taipei_bb_results$place_id[2])
ntpc_bb |> mapview()
```
...
```{r}
#| label: load-mosque-results
# saveRDS(
#   tpc_mosque_results,
#   'data/tpc-mosque-results.rds'
# )
tpc_mosque_results <- readRDS('data/tpc-mosque-results.rds')
```
...
# ky_results |> saveRDS('data/ky-result.rds')
```

```{r}
ky_results <- readRDS('data/ky-result.rds')
...
# ky_bookcase_results |> saveRDS('data/ky-bookcase-results.rds')
```

```{r}
ky_bookcase_results <- readRDS('data/ky-bookcase-results.rds')
